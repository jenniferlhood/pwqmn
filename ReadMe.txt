...
As the data is published near-annually as it becomes available, in Microsoft Excel spreadsheets (and Microsoft Access databases). Due to the format of the row entries of the data, I have found data exploration strictly using Microsoft products is not productive, and extracting very specific pieces of information is time consuming and prone to errors. Thus I have developed a tool which provides easy access to the PWQMN dataset for extracting some basic and specific information useful in data exploration and data mining. Future updates to this program will build on these basics and increase the statistical and graphical tools available to users.
